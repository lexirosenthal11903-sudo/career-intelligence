...
- [ ] **Networking & contacts layer** — _confirmed as a real feature (Lexi, 2026-06-22), not parked. Lives in the role-detail "Reaching out" section already built in the workspace panel (Step D)._ **Philosophy:** teaching someone to build a network is core mentorship and a real differentiator — the opposite of Jobcopilot's "blast 500 emails." We teach people to find and approach the right person, not hand them a list. Build as **two tiers**:
  - **Basic tier (build first — GDPR-safe today, no license, no stored third-party PII):** (1) advisor names *who to target by role* ("for this role the person who matters is usually the Head of [team] / hiring manager / recruiter") — knowledge, not PII; (2) generates a **deep-linked LinkedIn / Google search** scoped to that company + role so the user finds the person themselves; (3) advisor drafts the outreach message + coaches timing/etiquette/follow-up; (4) the user tracks who they've contacted (their own data). **Guardrail:** the advisor must NOT invent or assert a specific named individual — PII + accuracy/defamation risk. Target by role, hand off the search. Needs a `/brainstorming` or `/grill-me` spec session before build (UX of the hand-off + tracking).
  - **Licensed tier (fast-follow — ⚠️ gated on solicitor opinion):** surface the actual named contact *in-app* via a licensed vendor (Apollo / Hunter / RocketReach — never scrape LinkedIn ourselves; ToS + CFAA risk). Becoming a data controller requires: privacy-notice disclosure, legitimate-interest assessment, data-subject erasure handling, retention limits. Do not begin until legal clearance obtained (already on the pre-launch legal list).
