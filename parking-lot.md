...
| **Process / how-we-work sessions** | **"How we work" upgrade session (Lexi, 2026-07-05)** — spine = deep-research pass on **Anthropic's own published guidance** (Building Effective Agents · prompt-engineering guide · Claude Code best-practices · the "How Anthropic's Own Team Uses Claude Code" PDF on Lexi's desktop — some already adopted, see memory `feedback_claude_code_practices`), with adopt/skip/already-do verdicts; THEN layer in a time-boxed **buildwitholi (Oli Woodman)** scan. ⏰ **Next-session-start reminder:** ask Lexi to click "read it →" and share the content for **LLM Council** + **"get Claude to use your name"** (from Oli's welcome email) so we can assess them. Filter everything through our professional standards before adopting. |
